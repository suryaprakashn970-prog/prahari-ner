  ...
  if (!priorities || priorities.length === 0) {
    return (
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="text-sm text-gray-500 text-center">No immediate response priorities identified.</div>
      </div>
    );
  }
          ...
          {displayPriorities.map((priority, index) => (
            <div key={index} className={`p-3 rounded-lg border-l-4 ${
              ...
              <div className="bg-white p-2 rounded border border-gray-200 mt-2">
                <span className="text-xs font-semibold text-gray-700 block mb-1">Recommended Action:</span>
                <p className="text-xs text-gray-800">{priority.recommended_action}</p>
              </div>
            </div>
          ))}
        </div>
      ) : (
        <div className="text-sm text-gray-500 text-center">No response priority for selected zone.</div>
      )}
    </div>
  );
}
